...
def solve_heuristic():
    ...
    # ==========================================
    print("--- Phase 1: Solving Schedule and Room Allocations (Heuristic) ---")
    ...
    for g, g_subjs in group_subjects.items():
        if 1 < len(g_subjs) <= 6:
            print(f"Applying gap penalties for group {g} ({len(g_subjs)} subjects)")
            ...
            print(f"Skipping gap penalties for group {g} ({len(g_subjs)} subjects)")
    ...
    if status2 in [cp_model.OPTIMAL, cp_model.FEASIBLE]:
        print("Success! Heuristic solver works.")
