...
def solve_simplified():
    ...
    # ==========================================
    print("--- Phase 1: Solving Schedule and Room Allocations (Simplified) ---")
    ...
    # 2. Each subject assigned to exactly one room (no splitting)
    for s in subjects:
        model1.Add(sum(y[s["id"], r["id"]] for r in classrooms) == 1)

    # 3. Branch & Semester Parity (at most 1 exam per slot for each student group)
    for g, g_subjs in group_subjects.items():
        for t in slots:
            model1.Add(sum(x[sid, t["id"]] for sid in g_subjs) <= 1)

    # 4. Max 2 exams per day per student group
    for g, g_subjs in group_subjects.items():
    ...
    # 6. Room capacity limit per slot (expressed directly using z variables!)
    for r in classrooms:
        for t in slots:
            model1.Add(sum(subject_student_count[s["id"]] * z[s["id"], r["id"], t["id"]] for s in subjects) <= r["capacity"])

    # 7. Room Sharing constraints (max 2 subjects per room per slot)
    for r in classrooms:
        for t in slots:
            model1.Add(sum(z[s["id"], r["id"], t["id"]] for s in subjects) <= 2)

    # 8. Same-branch room sharing forbidden
    for r in classrooms:
        for t in slots:
    ...
    for r in classrooms:
        for t in slots:
            ...
            total_students_in_room = sum(subject_student_count[s["id"]] * z[s["id"], r["id"], t["id"]] for s in subjects)
    ...
    if status2 in [cp_model.OPTIMAL, cp_model.FEASIBLE]:
        print("Success! Simplified solver works.")
