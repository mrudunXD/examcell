...
def analyze():
    ...
    # We will try solving by enabling/disabling constraints
    for test_case in [
        "all_enabled",
        "skip_invigilator_constraints",
        "skip_backlog_separation",
        "skip_room_sharing_branch_mix",
        "skip_invigilator_and_room_sharing"
    ]:
        ...
        # 4. Backlog Separation
        if "skip_backlog_separation" not in test_case:
            backlog_subjects = [s for s in subjects if s.get("exam_type") == "backlog"]
            regular_subjects = [s for s in subjects if s.get("exam_type") == "regular"]
        ...
        if "skip_room_sharing_branch_mix" not in test_case and "skip_invigilator_and_room_sharing" not in test_case:
            for r in classrooms:
                for t in slots:
        ...
        if "skip_invigilator_constraints" not in test_case and "skip_invigilator_and_room_sharing" not in test_case:
            for f in faculty:
        ...
        print(f"Test case: {test_case} -> Status: {solver.StatusName(status)}")
