...
import { RefreshCw, TrendingUp, AlertCircle, Users, Scale, BarChart3, Activity, ShieldAlert, Award } from 'lucide-react';
import api from '../lib/api.js';
import toast from 'react-hot-toast';

function heatColor(value, max) {
  if (!value || max === 0) return { bg: 'rgba(255, 255, 255, 0.02)', text: 'var(--text-tertiary)' };
  const ratio = value / max;
  if (ratio < 0.25)  return { bg: 'rgba(48, 209, 88, 0.15)', text: '#30D158' };
  if (ratio < 0.5)   return { bg: 'rgba(10, 132, 255, 0.15)', text: '#0A84FF' };
  if (ratio < 0.75)  return { bg: 'rgba(255, 214, 10, 0.15)', text: '#FFD60A' };
  if (ratio < 0.9)   return { bg: 'rgba(255, 159, 10, 0.15)', text: '#FF9F0A' };
    ...
    <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 24, paddingBottom: 40 }}>
      ...
      <div className="page-header" style={{ marginBottom: 0 }}>
        ...
        <div className="flex-row" style={{ gap: 8 }}>
          ...
          <button
            className={`btn btn-sm ${view === 'charts' ? 'btn-primary' : 'btn-ghost'}`}
            style={{ borderRadius: 8 }}
      ...
      <div className="kpi-grid">
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <Award size={14} color="#BF5AF2" strokeWidth={1.5} />
          </div>
          <div>
            <div style={{ fontSize: 28, fontWeight: 700, color: '#BF5AF2', fontFamily: 'var(--font-mono)' }}>{fairnessScore}%</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
              ...
              <div className="flex-row" style={{ gap: 12, alignItems: 'center' }}>
                <span style={{ fontSize: 10, fontFamily: 'var(--font-mono)', color: 'var(--text-secondary)' }}>Load:</span>
                {[
                  { label: '0', bg: 'rgba(255, 255, 255, 0.02)', text: 'var(--text-tertiary)' },
                  { label: 'Low', bg: 'rgba(48, 209, 88, 0.15)', text: '#30D158' },
                  { label: 'Mod', bg: 'rgba(10, 132, 255, 0.15)', text: '#0A84FF' },
                  { label: 'High', bg: 'rgba(255, 214, 10, 0.15)', text: '#FFD60A' },
                  { label: 'Max', bg: 'rgba(255, 69, 58, 0.15)', text: '#FF453A' },
                ].map(l => (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
              <div>
                ...
                <p style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 2 }}>Search and inspect specific invigilator assignments and load status.</p>
              </div>
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                />
                <select className="select" style={{ fontSize: 12, padding: '4px 10px', width: 160 }} value={selectedCycle || ''} onChange={e => setSelectedCycle(e.target.value)}>
              ...
              <table style={{ minWidth: '100%' }}>
                ...
                <tbody>
                  {filteredFaculty.map(f => {
                    ...
                    const loadBadgeClass = total === 0 ? 'badge-ghost' : isOver ? 'badge-danger' : isUnder ? 'badge-warning' : 'badge-success';
                      ...
                      {(() => {
                        ...
                        return (
                          <>
                            ...
                            <path d={dLine} fill="none" stroke="#30D158" strokeWidth="2" />
                            {points.map((p, i) => (
                              <g key={i}>
                                <circle cx={p.x} cy={p.y} r="4" fill="#050505" stroke="#30D158" strokeWidth="2" />
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
                ...
                <div style={{ position: 'relative', width: 200, height: 130, display: 'flex', justifyContent: 'center' }}>
                  <svg width="200" height="120" viewBox="0 0 100 60" style={{ overflow: 'visible' }}>
                    <path d="M 10 50 A 40 40 0 0 1 50 10" fill="none" stroke="rgba(255, 69, 58, 0.2)" strokeWidth="6" />
                    <path d="M 50 10 A 40 40 0 0 1 82 26" fill="none" stroke="rgba(255, 214, 10, 0.2)" strokeWidth="6" />
                    <path d="M 82 26 A 40 40 0 0 1 90 50" fill="none" stroke="rgba(48, 209, 88, 0.2)" strokeWidth="6" />
                    {fairnessScore < 60 && (
                      <path d={`M 10 50 A 40 40 0 0 1 ${50 - 40 * Math.cos((fairnessScore / 60) * Math.PI * 0.5)} ${50 - 40 * Math.sin((fairnessScore / 60) * Math.PI * 0.5)}`} fill="none" stroke="#FF453A" strokeWidth="6" />
                    )}
                    {fairnessScore >= 60 && fairnessScore < 80 && (
                      <>
                        <path d="M 10 50 A 40 40 0 0 1 50 10" fill="none" stroke="#FF453A" strokeWidth="6" />
                        <path d={`M 50 10 A 40 40 0 0 1 ${50 + 40 * Math.sin(((fairnessScore - 60) / 20) * Math.PI * 0.2)} ${10 + 40 * (1 - Math.cos(((fairnessScore - 60) / 20) * Math.PI * 0.2))}`} fill="none" stroke="#FFD60A" strokeWidth="6" />
                      </>
                    )}
                    {fairnessScore >= 80 && (
                      <>
                        <path d="M 10 50 A 40 40 0 0 1 50 10" fill="none" stroke="#FF453A" strokeWidth="6" />
                        <path d="M 50 10 A 40 40 0 0 1 82 26" fill="none" stroke="#FFD60A" strokeWidth="6" />
                        <path d={`M 82 26 A 40 40 0 0 1 ${82 + 8 * Math.sin(((fairnessScore - 80) / 20) * Math.PI * 0.2)} ${26 + 24 * Math.sin(((fairnessScore - 80) / 20) * Math.PI * 0.2)}`} fill="none" stroke="#30D158" strokeWidth="6" />
                      </>
                    )}
